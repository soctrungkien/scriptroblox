...
--Script
local SectionScript = TabScriptAny:CreateSection("Script for " .. info.Name)
...
--Script Chung
local SectionScriptChung = TabScriptAny:CreateSection("Universal")
TabScriptAny:CreateButton({
   Name = "Infinite Yield",
   Callback = function()
...
TabScriptAny:CreateButton({
   Name = "Kiểm tra infU",
   Callback = function()
		loadsc("https://raw.githubusercontent.com/soctrungkien/1HzH9Axaj/main/uploads/testinfuv2.lua", "TestInfU-v2", true)
   end
})
TabScriptAny:CreateButton({
...
TabServer:CreateButton({
   ...
   Callback = function()
      ...
      local target = servers[1] -- nhiều nhất

      print("[infU] Join nhiều người:", target.playing)
	   Rayfield:Notify({
	      Title = "Đang vào server:",
	      Content = target.id,
	      Duration = 2.5,
